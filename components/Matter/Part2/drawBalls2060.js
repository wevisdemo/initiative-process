...
export const drawBalls2060 = (world, data) => {
  data.map((d) => {
    ...
    const line3 = Bodies.rectangle(elem[0].getBoundingClientRect().x - margin, elem[0].offsetTop + 12.5, 24, 1, {
      render: {
        fillStyle: d["ประเภท"] != "การเมือง" ? "black" : "white",
      },
      angle: Math.PI,
    });
    const line4 = Bodies.rectangle(elem[0].getBoundingClientRect().x - margin, elem[0].offsetTop + 12.5, 24, 1, {
      render: {
        fillStyle: d["ประเภท"] != "การเมือง" ? "black" : "white",
      },
      angle: Math.PI / 2,
